...
// Fetch canvas and its 2D context
const canvas = document.getElementById('screen');
const ctx    = canvas.getContext('2d');

const SCREEN_WIDTH = canvas.width = 200
const SCREEN_HEIGHT = canvas.height = 200

// Scale up for crisp, blocky look (optional)
ctx.scale(1, 1);
...
// Set a single pixel by its 0‑based index `n`.
// `color` is any CSS color string, e.g. "#0F0", "rgb(255,0,0)", etc.
function setPixel(n, color) {
  if (n < 0 || n >= SCREEN_WIDTH * SCREEN_HEIGHT) {
    console.warn(`Pixel ${n} is out of bounds (0–${SCREEN_WIDTH*SCREEN_HEIGHT -1})`);
    return;
  }
  // Compute x,y from index
  const x = n % SCREEN_WIDTH;
  const y = Math.floor(n / SCREEN_WIDTH);
...
// ── INTEGRATION WITH BINARYLANG ──────────────────────────────

// Extend your evalBinaryLang to recognize a new instruction:
//   SCREEN <reg-or-literal>, <color>
// e.g. SCREEN 0b1010, "#FF0"
const originalEval = evalBinaryLang;
evalBinaryLang = async function(input, registers) {
  const m = input.match(/^SCREEN\s+([A-Z]|0b[01]+|0x[0-9A-F]+|\d+)\s*,\s*["'](.+)["']$/i);
  if (m) {
    // parse pixel index
    let [, idxToken, color] = m;
    let idx;
    if (/^[A-Z]$/.test(idxToken)) {
      if (!(idxToken in registers))
        throw new Error(`Register "${idxToken}" not defined.`);
      idx = registers[idxToken];
    } else if (/^0b[01]+$/i.test(idxToken)) {
      idx = parseInt(idxToken.slice(2), 2);
    } else if (/^0x[0-9A-F]+$/i.test(idxToken)) {
      idx = parseInt(idxToken, 16);
    } else {
      idx = parseInt(idxToken, 10);
    }
    setPixel(idx, color);
    return `  SCREEN pixel ${idx} → ${color}`;
  }
  // Otherwise fall back to normal
  return originalEval(input, registers);
};
